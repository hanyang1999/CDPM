...
class CsubVPSDE(SDE):
  ...
  # def perturb_kernel(self, x, t):
  #   log_mean_coeff =  0.25 * t ** 2 * (self.beta_1 - self.beta_0) + 0.5 * t * self.beta_0
  #   f_t = torch.exp(log_mean_coeff)
  #   g_t = f_t - 1/f_t
  #   g_t += self.SDE_pre.sigma_min
  #   perturbed_mean =  x / f_t[:, None, None, None]
  #   # tau = self.SDE_pre.T *torch.log(1+g_t**2/self.SDE_pre.sigma_min**2)/np.log(self.SDE_pre.sigma_max ** 2/self.SDE_pre.sigma_min ** 2)
  #   # tau = torch.clamp(tau,min=0,max=self.SDE_pre.T)
  #   return perturbed_mean, g_t
